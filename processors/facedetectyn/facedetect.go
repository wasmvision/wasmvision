...
func process(image mat.Mat) mat.Mat {
	...
	for r := uint32(0); r < faces.Rows(); r++ {
		x0 := int32(faces.GetFloatAt(r, 1))
		y0 := int32(faces.GetFloatAt(r, 0))
		x1 := x0 + int32(faces.GetFloatAt(r, 3))
		y1 := y0 + int32(faces.GetFloatAt(r, 2))

		faceRect := types.Rect{Min: types.Size{X: x0, Y: y0}, Max: types.Size{X: x1, Y: y1}}

		rightEye := types.Size{
			X: int32(faces.GetFloatAt(r, 5)),
			Y: int32(faces.GetFloatAt(r, 4)),
		}

		leftEye := types.Size{
			X: int32(faces.GetFloatAt(r, 7)),
			Y: int32(faces.GetFloatAt(r, 6)),
		}

		noseTip := types.Size{
			X: int32(faces.GetFloatAt(r, 9)),
			Y: int32(faces.GetFloatAt(r, 8)),
		}

		rightMouthCorner := types.Size{
			X: int32(faces.GetFloatAt(r, 11)),
			Y: int32(faces.GetFloatAt(r, 10)),
		}

		leftMouthCorner := types.Size{
			X: int32(faces.GetFloatAt(r, 13)),
			Y: int32(faces.GetFloatAt(r, 12)),
		}
